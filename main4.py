...
class ObjectTracker:
    ...
    def update(self, detections):
        ...
        for obj_id, tracker in self.trackers.items():
            ...
            h = tracker.h
            des = tracker.des
            coords[obj_id] = [x - w // 2, y - h // 2, w, h, des]
...
def detect_squares(frame):
    ...
    for contour in contours:
        ...
        if len(approx) == 4:
            ...
            # Optionally, check if the aspect ratio is close to 1 (for squares)
            aspect_ratio = float(w) / h
            if 0.9 < aspect_ratio < 1.1:
                # Append the bounding box information: [x, y, width, height]
...
while True:
    ...
    detections_squares, gray = detect_squares(frame)
    ...
    for box_id in boxes_ids:
        ...
        if r == 0:  # It's a square
            ...
            cv2.putText(frame, str(obj_id), (x, y - 15), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 0), 2)
            cv2.circle(frame, (x + r, y + r), r, (0, 255, 0), 3)  # Draw the circle
